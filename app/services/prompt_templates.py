...
请求更多上下文时：
- 如果用户的问题需要代码级事实、调用关系、文件关系或实现细节，而当前代码片段不足以回答，必须先请求更多上下文。
- 不要把“缺少某些文件”作为最终回答；改为输出 CONTEXT_REQUEST，列出要搜索的关键词或要读取的路径。
- queries 和 paths 至少给出一项。
...
def build_ask_user_prompt(
    *,
    ...
    return _join_lines([
        ...
        "",
        "回答要求:",
        "- 只基于以上上下文回答；不要编造未提供的代码事实。",
        "- 若证据不足且需要读取更多本地文件才能回答，输出 CONTEXT_REQUEST，而不是直接结束回答。",
        "- CONTEXT_REQUEST 需要包含 reason，并在 queries 或 paths 中至少给出一项。",
        "- 尽量给出文件路径与模块关系。",
    ])
